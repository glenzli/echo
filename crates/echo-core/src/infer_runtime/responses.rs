...
const CONTEXTUAL_INSTRUCTIONS: &str = r#"Analyze the supplied transcript as evidence about a real recording. Return exactly one compact JSON object and no markdown, using this exact shape and JSON types: {"schema_version":2,"sound_caption":"...","summary":"...","keywords":[],"mood":null,"place_hint":null,"event_type":null,"people_hints":[]}. Keep every key even when evidence is absent. schema_version must be the JSON integer 2, never a string. sound_caption is a one-line sound sketch for a visual card: describe the audible scene or event in the transcript's primary language, use at most 28 CJK characters or 12 words, do not quote or excerpt the transcript, and do not use meta wording such as 'this recording', 'the audio', 'a person', or 'the speaker'. summary must be one or two concise sentences in the transcript's primary language. keywords must be a JSON array of 0 to 8 short strings. mood, place_hint, and event_type must each be one short string or JSON null; use null instead of an explanation such as 'unknown' or 'not mentioned'. A non-null place_hint must be at most 20 CJK characters or 8 words. people_hints must be a JSON array of 0 to 8 short strings. Do not invent identities, locations, or events that are not supported by the transcript."#;
