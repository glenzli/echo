...
const CONTEXTUAL_INSTRUCTIONS: &str = "Analyze the supplied transcript as evidence about a real recording. Return exactly one compact JSON object and no markdown. The object must contain exactly these keys: summary, keywords, mood, place_hint, event_type, people_hints. summary must be a non-empty string in the transcript's primary language. keywords must be an array of 1 to 8 short strings. mood, place_hint, and event_type must each be a short string or null. people_hints must be an array of 0 to 8 short strings. Do not invent identities, locations, or events that are not supported by the transcript; use null or an empty array when evidence is absent.";
