...
use crate::{
    ...
    schema::{
        ...
        RENDERED_SPECTRAL_WORKING_COPY_EXPORT_MIGRATION_SQL,
        RENDERED_SPECTRAL_WORKING_COPY_EXPORT_SCHEMA_VERSION,
...
fn initialize_schema(connection: &Connection) -> Result<(), CatalogError> {
    ...
    match stored_version {
        ...
        {
            // Add disclosure history and the current analysis vocabulary; old binaries must not
        ...
        Some(version)
            if version
                .parse::<CatalogSchemaRevision>()
                .is_ok_and(|revision| {
                    revision == RENDERED_SPECTRAL_WORKING_COPY_EXPORT_SCHEMA_VERSION
                }) =>
        {
            migrate_rendered_spectral_working_copy_export_schema(connection)?;
        }
        Some(version)
            if version
                .parse::<CatalogSchemaRevision>()
        ...
        Some(version)
            if version
                .parse::<CatalogSchemaRevision>()
                .is_ok_and(|revision| revision == DRIVE_ROTARY_SCHEMA_VERSION) =>
        {
            migrate_convolution_space_schema(connection)?;
        }
        Some(version)
            if version
                .parse::<CatalogSchemaRevision>()
        ...
        Some(version)
            if version
                .parse::<CatalogSchemaRevision>()
                .is_ok_and(|revision| revision == SPACE_CHARACTERS_SCHEMA_VERSION) =>
        {
            migrate_creative_vfx_schema(connection)?;
        }
        Some(version)
            if version
                .parse::<CatalogSchemaRevision>()
        ...
        Some(version)
            if version
                .parse::<CatalogSchemaRevision>()
                .is_ok_and(|revision| revision == PROCESSING_RECIPES_SCHEMA_VERSION) =>
        {
...
fn migrate_rendered_spectral_working_copy_export_schema(
    connection: &Connection,
) -> Result<(), CatalogError> {
    let transaction = connection.unchecked_transaction()?;
...
fn migrate_convolution_space_schema(connection: &Connection) -> Result<(), CatalogError> {
    let transaction = connection.unchecked_transaction()?;
    transaction.execute_batch(CONVOLUTION_SPACE_MIGRATION_SQL)?;
    finish_migration(&transaction)?;
    transaction.commit()?;
    Ok(())
}
...
fn migrate_creative_vfx_schema(connection: &Connection) -> Result<(), CatalogError> {
    let transaction = connection.unchecked_transaction()?;
    finish_migration(&transaction)?;
    transaction.commit()?;
    Ok(())
...
fn migrate_processing_recipe_management_schema(
    connection: &Connection,
) -> Result<(), CatalogError> {
    let transaction = connection.unchecked_transaction()?;
...
fn apply_rendered_spectral_working_copy_export_migration(
    transaction: &rusqlite::Transaction<'_>,
) -> Result<(), CatalogError> {
    let table_count: i64 = transaction.query_row(
        "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' \
         AND name = 'render_export_working_copy_provenance'",
        [],
        |row| row.get(0),
    )?;
    match table_count {
        0 => transaction.execute_batch(RENDERED_SPECTRAL_WORKING_COPY_EXPORT_MIGRATION_SQL)?,
        1 => {}
        _ => {
            return Err(CatalogError::new(
                CatalogErrorKind::SchemaMismatch,
                "catalog has an ambiguous rendered spectral working-copy export schema",
            ));
        }
    }
    Ok(())
...
fn apply_creative_vfx_migration(
    transaction: &rusqlite::Transaction<'_>,
) -> Result<(), CatalogError> {
    let column_count: i64 = transaction.query_row(
        "SELECT COUNT(*) FROM pragma_table_info('asset_adjustment_revisions') \
    ...
    if column_count == 0 {
        transaction.execute_batch(CREATIVE_VFX_MIGRATION_SQL)?;
    }
    Ok(())
}
...
fn apply_processing_recipe_management_migration(
    transaction: &rusqlite::Transaction<'_>,
) -> Result<(), CatalogError> {
    let archived_column_count: i64 = transaction.query_row(
        "SELECT COUNT(*) FROM pragma_table_info('processing_recipes') \
         WHERE name = 'archived_at_millis'",
        [],
        |row| row.get(0),
    )?;
    let revert_table_count: i64 = transaction.query_row(
        "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name IN (\
    ...
    match (archived_column_count, revert_table_count) {
        (0, 0) => transaction.execute_batch(PROCESSING_RECIPE_MANAGEMENT_MIGRATION_SQL)?,
        (1, 2) => {}
        _ => {
            return Err(CatalogError::new(
                CatalogErrorKind::SchemaMismatch,
                "catalog has an incomplete processing recipe management schema",
            ));
        }
    }
    Ok(())
