...
use crate::{
    ...
    schema::{
        CatalogSchemaRevision, LOW_CUT_MIGRATION_SQL, PREVIOUS_SCHEMA_VERSION, SCHEMA_IDENTITY,
...
fn initialize_schema(connection: &Connection) -> Result<(), CatalogError> {
    ...
    match stored_version {
        ...
        {
            migrate_low_cut_schema(connection)?;
        }
        Some(version) => {
...
fn migrate_low_cut_schema(connection: &Connection) -> Result<(), CatalogError> {
    let transaction = connection.unchecked_transaction()?;
    transaction.execute_batch(LOW_CUT_MIGRATION_SQL)?;
    transaction.execute(
