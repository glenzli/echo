...
fn revision_round_trips_in_date_dot_sequence_form() {
    assert_eq!(SCHEMA_VERSION.to_string(), "20260809.6");
    assert_eq!(
        CatalogSchemaRevision::from_str("20260809.6"),
        Ok(SCHEMA_VERSION)
    );
    assert_eq!(SCHEMA_VERSION.date(), 20_260_809);
    assert_eq!(SCHEMA_VERSION.daily_sequence(), 6);
}
