...
Echo 是一款 local-first 的声音资料库、非破坏性声音调整与声音编排应用。它保存、理解、整理、修复并帮助人重新聆听真实发生过的声音；Library 是一等公民，调整和编排都是 Library 的能力。
...
- **编排保持可追溯**：多轨编排只使用资料库声音；每个片段固定准确的 Original 与调整版本，编排版本和混音导出不回写单条声音。
...
Echo is a local-first sound Library, non-destructive sound-adjustment, and sound-assembly application. It preserves, understands, organizes, restores, and helps people revisit sounds that actually happened. The Library is the primary product; adjustment and assembly are capabilities of that Library.
...
- **Traceable assembly**: multitrack assemblies only use Library sounds. Every clip pins an exact Original and adjustment revision; assembly revisions and mixdowns never write back into a single sound.
