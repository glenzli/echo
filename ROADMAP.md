...
记忆库是日常收藏、搜索和重温的入口；完整多轨编辑与精细声音处理服务于这些内容。素材是否进入记忆库由用户决定，外部声音不再被强制当作值得长期重温的记忆。录音、MIDI 与插件宿主仍保持独立里程碑。生成式补充允许沿显式、克制、可追溯的路径进入工程与记忆；生成入口须与来源标记和重温筛选一起交付。
...
6. **调整克制**：恢复性调整（Loudness/EQ/降噪/去混响/Dynamics/Trim/Fade/Channel）与
   ...
   Restoration。生成补充属于独立来源职责：用户主动指定用途／范围，试听候选后显式采纳；
   环境声补齐、短缺口补全和延长可以逐步接入，所有新生成内容必须显著标记。对白重建单独
   标记，不能成为原录音转写或现场事件证据。AI 降噪、源分离和生成执行分别验证 Infer 的真实合同。
...
### 2026-09-22：克制的生成补充与来源披露

记忆／素材表示保存和使用意图，AI 处理／生成补充／重构对白表示来源性质。不可变文件引用
只证明字节身份，不证明声音来自现场。生成补充不覆盖 Original，不自动采纳或替换已保留版本；
已采纳结果是受管持久素材，不能当作重新推理即可恢复的临时缓存。生成过程的实际输入、模型与
结果身份需由后续执行器写入，用户手动说明不冒充模型执行证据。

首批落地用户声明的来源区间：可给已导入声音的完整时长或原始时间选区标注 AI 处理、AI 生成
补充、重构对白及说明。Catalog 追加带内容 hash 的披露修订，带预期版本校验；清除标记保留历史。
旧库没有标记表示未声明，不能据此声称已验证实录。工程副本携带同一台账。素材／记忆身份不变。

当前标记保守地描述“来源含 AI”：编排引用的非静音来源传递其披露，避免在效果尾音或局部裁切
后轻率断言已消除生成成分；来源区间不冒充最终混音中精确的声学影响范围。已保存编排按固定
revision 的引用判定，当前草稿不能改写旧版本；导出保存当次来源声明快照。细粒度影响映射与
历史聆听版本选择属于后续独立切片。

磁带提供“包含 AI 生成来源”，默认关闭；原始磁带始终排除已声明的生成来源。关闭时按完整
声音条目筛选并显示排除数量，不临时静音、重混或悄悄替换聆听版本。AI 处理与生成分别呈现，
确定性 EQ／剪辑／去爆音不被归为生成。来源标记须在卡片、磁带和编辑器可见；自动分析描述
生成音频时不得提升为现场发生的事实。本批不新增模型或生成执行入口。

### 2026-09-22：音频文件携带来源声明

单音、修音工作副本、多轨 WAV 与批量 WAV／FLAC 导出在容器 comment 中携带
`echo.source-disclosure.v1`：仅含去重的来源类别，范围固定为 referenced_sources。
WAV 使用标准 LIST/INFO/ICMT，FLAC 使用 comment；PCM 不变，不携带路径、来源 ID、私人说明
或提示词。未标记表示未知，不是实录认证；声明不具签名认证能力，外部转换可能移除元数据。

导出开始时捕获类别，记录发布时在同一 Catalog 事务中核对；类别变化时不写矛盾的导出记录，
提示重新导出。批量恢复沿用落盘的原声明，旧恢复项缺少声明不能被误报为已披露成功。
重新导入支持的声明时保守标为整个文件，并区分文件携带与用户声明；用户校正／清除写入追加
台账并优先于嵌入值，刷新源元数据不得复活已清除标记。未知版本／坏格式不被猜测成有效来源。
该切片完善外部文件往返；模型候选、采纳及执行证据仍归后续生成执行职责。
...
- `sound_items` 负责记忆／全局素材的用户收集身份，引用真实 `AudioAsset` 或独立 `SoundAssembly`。
  ...
  收藏、评分、声音集和聆听状态使用统一声音条目身份；旧原录音、调整与 Analysis 字节不改写。
...
### 2026-09-20：组合回归与 AI 检索边界

本轮暂停扩展产品能力，验证复杂编辑、异步取消、保存重开、输出一致性和实际本地推理。
文本与 CLAP 仍使用独立、精确的向量空间，不把不同模型的余弦分数当作可比较的置信度。
已失联或内容身份过期的 CLAP 索引不参与搜索。模型标签和自然语言检索仍是辅助线索；
本轮真实夹具暴露了事件误判及融合检索对双索引素材的偏重，不能据执行成功声称质量普遍达标。
简单平均融合通道会改善单索引旁白，却降低另一部分非语音查询质量，因此不纳入产品。
后续检索质量校准需要覆盖语音、非语音和中英文的独立标注样本，保留本轮失败案例。

CLAP consumer 固定到 SDK 修复 `129ce3a30518bf85980d89e130420fe8f52530f1`，补齐
`infer.audio.embedding@20260815.2` 的精确 schema 登记。旧库中因 SDK 返回
`consumer_core_unsupported (ContractMismatch)` 而失败、且仍缺少有效音频索引的任务，
在原有任务身份上恢复一次，并在任务 payload 保存 `clap_schema_retry` 标记。
再次失败、其他错误、已取消或已建立索引的任务不自动重试；原录音和已有向量不改写。

## 3. 技术架构

### 编辑流程收口（2026-09-22）

主窗口离开单声音编辑前显式处理未保存调整：保存成功才继续，放弃仅还原草稿，取消保留
当前编辑；关闭窗口同时保存多轨工程草稿，不替换记忆库的聆听版本。
`UnsavedEditDialog` 负责离开确认，实际保存仍由编辑器持有。素材试听固定到被试听来源，
不随列表选择变化；“添加声音”展开并聚焦素材搜索。生成旁白采纳后可直接定位素材，
项目内生成默认仅随项目保留，全局收集显式选择；素材行持续披露 AI 来源，完整生成记录
按需展开。上述改动不改变 Original、调整 revision 或已保留混音的身份。

### 2026-09-22：持续试听中的轨道混音

固定来源、片段位置和处理版本的试听允许在播放中调整轨道音量、声像、静音、独奏及主音量。
控制快照有界、合并到最新值，只在 producer 的块边界接收；设备 callback 仍只复制音频。
参数用 10 ms 渐变衔接，保留当前播放位置与已有有界缓冲，不重新准备来源。轨道电平显示
producer 侧推子后、主处理前的立体声峰值；它受预读缓冲影响，不冒充设备输出时刻的测量。
拖动中的值是临时试听状态，松手才写入草稿并形成一次撤销。撤销与重做同样可在试听中生效。
来源、片段或 limiter 设置改变仍使试听计划失效；参数更新不修改 Original、固定来源版本或
已保存的记忆聆听版本。持久化继续使用现有混音字段，本阶段不扩展项目格式或引入新模型。

### 2026-09-22：编排标记与命名区间

`SoundAssembly.markers` 保存最多 256 个用户命名的时间点或区间，使用独立 UUID 和编排毫秒
坐标；旧文档缺少此字段时视为空。标记属于工程注释，不是音频来源或 AI 分析，不参与 DSP。
它们随工程版本、撤销和便携项目保存；不会改变音频时长、Original 或记忆库聆听版本。
移动、裁切或波纹删除片段时，标记仍固定在编排坐标，避免把用户命名的时间悄悄重新解释。
标记时间限制在四小时内；缩短音频后超出音频的标记保留并提示，不提供无效区间试听。
编辑器提供标记列表、时间尺位置、前后导航和命名区间循环试听。标记名称变化不中断播放；
当前试听区间的边界变化则使原试听窗口失效。持久化沿用编排 JSON，不新增数据库表。

### 2026-09-22：文字批量编辑与语音间隔复核

文字面板支持搜索、逐条勾选和批量隐藏／仅保留勾选声音。每次批量动作先合并重叠的原始时间
区间，完整校验后一次发布到现有源编辑草稿，只有一个撤销步骤；不修改原始音频或转写证据。
仅保留不恢复此前隐藏的声音；空输出和超过 128 段的操作整笔拒绝，不截断或重置已有编辑。
搜索、证据、显示粒度或编辑版本变化时清空勾选，防止过滤列表之外的隐含删除。

语音间隔候选只来自有时长的对齐单元之间，重叠单元先形成语音覆盖；零时长单元作为阻挡
边界，不能凭缺失时长推断可删除区间。候选保留两端余量，支持最小时长过滤、原声带上下文
试听和手动采纳。它是 AI 时间证据的编辑辅助，不是静音／无用声音判断，环境声可能仍在其中。
此切片复用已有 Infer ASR／对齐合同，不增加模型、库后台任务或新的持久化事实类型。

### 2026-09-22：短爆音检查与局部修复

去爆音局部作用前，固定延迟环同步原声、处理声和来源时间；旁路、重置与尾部补偿沿用同一时间轴，实时回调不分配内存。

用户显式检查最多五分钟的 Original 选区，后台复用现有 `DeClickProcessor`，比较其延迟对齐
后的输入与输出，返回有界疑似短爆音列表。它是确定性修复算法的候选，不是 AI 分类或缺陷事实；
乐器瞬态也可能命中。扫描保留声道信息、原始时间及两端真实解码上下文，最多显示 256 处，
取消和来源／检测参数变化后不得采纳迟到结果。分析只用固定解码块与短历史，不缓存整段 PCM。

选中候选可以定位、带上下文试听原声，并显式添加去爆音的局部作用区间。新节点置于效果链
起始处；已有启用的局部去爆音只增加区间，不缩小原作用范围或改写参数。全局已启用时不重复
添加；已有旁路局部设置时要求在效果面板复核，不自动唤醒其旧区间。最多 64 个效果作用区间，
超限整笔拒绝。一次采纳形成一个可撤销草稿动作，沿用现有修复参数和效果遮罩持久化，不改
Original、已保留聆听版本或项目格式。调整后试听／导出继续使用同一效果执行链。

### 2026-09-20：编辑自动化与项目内 AI

按编辑反馈、可编辑自动化、文字/语义操作、候选修复的顺序交付。片段音量包络以准备后来源
时间和 centibel 为持久坐标；移动、分割与裁切不重新解释曲线。旧工程没有包络时保持原声音。
试听与导出共享包络执行，自动闪避只生成用户可编辑、可旁路、可撤销的曲线，不冒充人声分离。

准备后的来源按真实媒体内容、完整处理参数和执行版本复用，片段位置与混音参数不使其失效。
缓存由会话拥有，写入完成前不可命中，取消与失败不得留下半成品；按资源大小清理不再使用的
条目。长工程先提供有界选区试听，再演进共享混音计划的分块播放，不能把离线试听称为实时混音。

项目内 AI 为用户显式动作，结果随当前工程保存，不因独立编辑而启动全库扫描。转写/对齐与
语义检索复用 Infer 的既有合同；文字选择映射回真实源时间，低粒度证据不伪装成逐字对齐。
新增神经增强和源分离需要独立的 Infer 音频结果合同、真实模型评测和候选采纳流程。
每个里程碑以真实编辑、撤销、保存重开、取消和音频交付验证收口。

### 2026-09-21：共享混音执行与分块试听

`AssemblyMixer` 在生产线程按固定 4096 帧执行片段来源、音量包络、声像、轨道混合和 Master
limiter，离线导出与流式试听使用同一分块边界和执行状态。`AssemblyPlaybackSession` 只预混
16384 帧（约 341 毫秒），设备回调只读取固定环形缓冲；不再先生成整段混音 WAV。
已预处理的 PCM 音源由 `PreparedPcmReader` 在混音线程内直接分块读取，避免每个片段
重复创建解码线程和完整 DSP 会话；其他格式保持原解码路径。覆盖 8 轨、256 个重叠片段、
长时间空白、连续定位、截断音源及试听／导出一致性。
首次试听仍须准备各来源的固定处理版本，后续使用内容缓存。活跃试听引用的来源全部固定保留，
包括尚未播放的片段，导出后的缓存清理不能删除它们。参数编辑仍停止旧试听并准备新文档，
本切片不提供播放中修改混音参数。定位通过关闭读取、清空旧缓冲并重置 DSP 状态执行；
缺失来源和解码错误返回界面，暂停和播放结束后的生产线程休眠，不持续轮询。

独立编辑与记忆库窗口共用 44 px 自定义标题栏、图标导航与选中指示。独立窗口顶部的模式、
撤销、重做、打开、保存和导出采用图标，保留工具提示与无障碍名称；编辑历史仍由当前工作区拥有。

### 独立编辑

`--edit` 与 `--project` 直接进入独立编辑窗口，复用单音处理、多轨、试听与导出。
每个窗口使用私有工作目录、项目 Catalog 和缓存，不打开全局记忆库、不登记扫描根目录、
不启动 Library worker、自动转写或语义索引。Catalog 在这里仅是工程持久化格式；
素材的接纳、波形生成和工程保存由独立编辑会话显式发起。

`.echo` 工程是带版本标记的 SQLite 快照，分块携带项目拥有的素材和处理文件。
工程内使用相对路径；重开先校验并展开到新的私有工作目录，原工程直到显式保存才更新。
保存通过同目录候选文件完成后原子替换，原始输入文件永不覆盖。独立编辑保留未正常关闭
...
成组移动、跨轨移动、复制、分割和删除整体提交，一次操作只有一个撤销步骤；来源版本和包络
仍锚定准备后的源时间。波纹删除移除所选片段时间区间的并集，可明确作用于选中轨道或所有轨道；
跨越区间的其他片段保留左右来源区间，间隙不被额外吞并。全部删除、超出片段容量或时间边界时
整笔操作拒绝。选择不成为新的持久化分组，不改变旧工程格式。

反复试听的会话生命周期同时收紧：音频设备继续复用，旧会话只保留到正在进行的回调读取结束，
再由主线程回收；停止后仍有短暂回收任务，完成即关闭。回调不执行分配、引用计数或析构，
避免长时间编辑把所有历史解码器及音频缓冲一直保留到窗口退出。
...
- 开发调试可以获取有来源和许可的音频，并用 Infer Runtime 独立的本地操作身份生成明确标记
  的合成素材，记录请求、模型／作业身份与文件哈希。调试生成与产品生成入口分别验证；产品入口遵循来源披露和候选采纳合同。
...
后续独立里程碑仍包括上下文修补与克隆、音符级音高校准与保调时间伸缩、
轨道效果链和自动化。现有移调、底噪扩展器及矩形衰减不能冒充这些能力；每项以实际编辑、试听、
保存重开和交付验证收口。

### 2026-09-20：噪声样本学习与频谱降噪

频谱工作区增加按需展开的噪声修复检查器。用户显式选择 0.1–30 秒的纯底噪区间，
从 Original 学习 48 kHz / 2048 点 Hann STFT 的平均功率谱；捕获样本本身不启用处理。
学习结果、来源区间、算法版本和降噪参数随原资产调整版本保存，旁路、清除、撤销与重开均可恢复。
旧版本没有噪声样本时保持原序列化字节，不迁移原始媒体。

降噪采用有最大衰减下限、频率和时间平滑的频谱功率估计，放在原始频谱修复阶段，
与手工区域衰减组合。双声道共用增益以保持相位与声像。频谱窗口在首尾补齐重叠并丢弃填充输出，
避免起始边界的归一化放大，保留逐帧来源位置和原长度。编辑试听、响度分析、单文件导出
及编排中固定版本的准备使用同一执行路径。被移除声音的监听只使用原始源与暂态差值滤波，
不写入调整版本或交付文件；参数、来源或页面变化使旧诊断试听失效。

样本分析有界、可取消，完成结果只能回到发起它的来源与草稿；原录音永不覆盖。
该能力针对稳定底噪，界面明确提示样本中的人声可能被当作噪声衰减，不声称能修复缺失内容、
智能分离动态背景或执行音符级修音。
...
备注、命名地点、描述性时间及原始时间轴上的片段注记属于独立用户事实，以 append-only 修订
保存在 Catalog；独立编辑时随 `.echo` 工程保存，不修改 Original，也不覆盖文件标签或模型摘要。
时间允许近似描述，不推导为精确录制时间；地点允许日常名称，不要求坐标。整份编排可有自己的
记忆信息，但不继承某一个片段的地点；片段的原始信息仍归其源资产。并发编辑按修订号拒绝覆盖。
导出默认排除个人信息；显式选择后，WAV、FLAC、MP3、M4A 将备注、地点和时间写入 comment，
FLAC 另写 LOCATION。源时间片段注记只留在 Echo，避免裁剪、多轨后产生错误的时间锚点；
AI 来源声明保持独立语义并始终随导出保留。本阶段不做地图查找或坐标反查。
...
### 选区转写的证据边界（2026-09-20）

显式转写最多五分钟的原始声音区间，复用 Infer 的本地 ASR 与来源代理；结果带来源内容 hash、原始区间和实际模型/Job 身份。`SelectionTranscript` 单独持久化，不满足整段 ASR、摘要或索引的完成条件。界面在来源、选区或处理版本变化后丢弃迟到结果；只有当前请求可以接受证据。文字动作使用原始时间，按句段选择、试听、隐藏或保留，音频操作进入已有撤销历史。当前同步 ASR SDK 在返回前不暴露 Job ID；放弃结果不宣称已经取消 Infer 的计算。独立模式只操作私有工程 Catalog，不启动库工作池。

源处理改变裁剪或隐藏区域时，片段增益曲线通过 Original 区间重新映射到 prepared source 时间，保留幸存音频的增益；新恢复的音频使用 0 dB，原曲线仍可从旧编排版本恢复。拼接处最多使用持久化网格的 1 ms 过渡；超过关键点上限时整笔版本保存失败，不截断作者数据。
...
### 2026-09-22：工程历史比较与旁白多候选

工程历史读取 Catalog 的不可变编排修订，按每页 50 条倒序获取摘要，选中时才解析精确来源。
`AssemblyHistoryDialog` 与 `AssemblyVersionComparison` 拥有比较呈现和按需试听：显示当前草稿
相对已存版本的轨道、片段位置／范围／处理版本／混音／包络及标记变化，独立播放通道支持
相同时间位置 A/B。比较不保存草稿、不发布新修订、不更换记忆库聆听版本。恢复旧版本进入
现有撤销历史，保留旧来源的精确调整版本；随后保存才追加修订。历史记录不冒充已保留的聆听版本。

旁白生成器最多持有三个独立临时候选，逐次显式生成；选中、移除、失败重试与生成记录由
`GeneratedNarrationController` 统一持有。失败保留此前候选，每个候选分别试听后才允许采纳，
采纳捕获选中候选，结束后释放整组临时文件。编辑输入只作用于下一次生成；不批量调度模型，
不保证多个执行产生不同内容，关闭面板仍丢弃候选并忽略迟到结果。

### 2026-09-22：音频格式与统一交付

扫描器、文件选择器共用 `audio_formats` 的扩展名候选表，覆盖常用无损、有损、录音与视频容器；
实际可用性仍以 FFmpeg 探测、解码为准。MKA／WebM 的轨道标签与容器时长由同一输入 owner
解析，禁止把未知时长当成无符号超长音频。手动导入、拖放和独立编辑命令行入口显式选择
多音轨容器中的一条音频轨；单轨保持原路径。选轨先按 SHA-256 完整保留容器，再无重编码
复制为 MKA，写入原容器身份、原名与 stream index。工程保存携带完整容器与选轨文件，
不改原字节、不自动提升为记忆；目录自动扫描暂时仍选第一条音频流。

单段、频谱工作副本、多轨与批量交付共用 WAV PCM16／PCM24／float32、FLAC24、MP3、M4A AAC
编码 owner，支持 44.1／48／96 kHz（有损格式为 44.1／48）、单／双声道，以及有损 128／192／
256／320 kbps。现有 DSP 保持 48 kHz 双声道，输出采样率是交付重采样，不宣称原生 96 kHz
处理。WAV 使用 FFmpeg RF64 auto；有损响度测量描述编码前 PCM。保存到记忆与内部预览仍使用
原有无损工作规格，外部交付设置不改变原始声音或工程处理精度。

Catalog `20260922.3` 原子扩展交付格式约束并保留旧记录 ID、来源引用和已保留聆听版本。
压缩格式 bit depth 记为 0，真实交付采样率、声道、帧数与内容身份随输出持久化；批量恢复
保存完整输出选项。所有输出格式沿用显著的生成来源声明和重新导入传播。格式回归使用本地
确定信号；APE／AMR 和实际超过 4 GiB 的物理导出暂不纳入自动样本矩阵，不把扩展名候选冒充
已验证的完整格式兼容性。

语音转写／对齐对模型不直接接受的容器使用短生命周期 16 kHz 单声道 WAV 代理，复用现有
十分钟有界解码。代理在请求结束或失败后清理，生成前后核对 Original 内容身份，不作为新
记忆或素材入库。分析代理按输出帧时间裁剪并刷出重采样尾部；来源时间仍锚定原始声音。

ADTS AAC／ASF WMA／Opus 的编码延迟、预跳样本与估算时长不能作为精确编辑边界；首次探测
以有界缓冲解码获得实际样本数与起点，最多缓存 32 个文件身份对应的标量结果。首次探测可能
比只读头部慢，后续试听与导出复用结果，不保留整段 PCM。压缩音频定位先读入解码所需的
前置帧，再按原始时间发布目标范围，避免整段或中途裁剪交付出现短读。

对毫秒级容器时间戳，首次从非零位置裁剪采用解码并丢弃前缀保证交付样本完整；交互定位
仍使用索引和解码预读。此类长文件从较晚位置裁剪会增加准备时间，内存使用保持有界。

### 2026-09-22：独立编辑首页与最近工程

首页将打开音频、打开工程和生成旁白合为一排入口，下方直接呈现有界、虚拟化的最近工程列表。
已保存工程在成功打开／保存时写入最多 40 条的本地路径索引，按最近使用时间排序，正常关闭后
仍可重开；这是独立编辑历史，不创建记忆库身份。多窗口使用锁和原子替换更新索引；离线／移动
的文件暂不显示，不主动擦除记录。测试环境的索引跟随隔离会话根目录。

可恢复编辑与已保存工程明确区分，前者显示工程／声音名称、来源数量和 Catalog／WAL 的
实际修改时间；两者按各自的时间合并显示。后台扫描只读摘要，不迁移工程；忽略空工程、正在
使用的工程和非独立编辑目录，不删除恢复数据。锁文件的创建时间不能冒充编辑时间。
桌面自动验证的工程目录随测试素材隔离，手动调试也可指定独立会话目录，避免污染用户恢复历史。

### 2026-09-22：转写交付与工程编辑状态

转写面板支持复制完整识别文本及 TXT／SRT／WebVTT 导出。字幕以当前证据记录的原录音时间为准，
不冒充剪辑后成片的字幕；缺少有效句段时间时仍可阅读和导出文字，不推造时间。
`TranscriptExportMenu` 拥有导出选择与固定快照，`TranscriptExportController` 拥有格式验证、
原文件保护与原子写入。操作不修改录音或转写证据。

载入来源草稿期间不发布脏状态，恢复参数时合并派生比较；只读资产刷新不再标记工程修改，
实际写入通过 `projectContentChanged` 单独通知，生成素材采纳仍计入工程修改。

### 2026-09-23：精确导航与选中转写交付

`Timecode` 与 `TimeRangeDialog` 拥有秒／时分秒解析、毫秒精度、当前来源边界和上下文失效。
单音入口只建立选区并适配视图，多轨入口只定位播放头，均不触发文档修改或自动播放。裁剪
改变后，选区约束到仍可用的来源；空选区同时关闭循环。时间轴的播放与 I/O 快捷键不截获
带 Ctrl／Command／Alt 的系统输入。

转写交付可选完整记录或当前勾选的句段／对齐单元，保留原始时间，不应用编辑边界余量，
也不填补被排除的文字。打开保存对话框时固定所选文字与来源；后续筛选不改变正在导出的
内容。超过五分钟的选区在提交推理前给出说明。
