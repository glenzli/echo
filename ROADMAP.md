...
- 声音墙的排序、筛选、结果计数、网格／单声音视图与卡片密度属于全局浏览状态，放在跨工作区
  底部工具栏；卡片尺寸滑块以接近目标宽度的列数排布，默认应形成紧凑并行浏览，而不是宽卡片。
  Like、评分与展开属于所选声音的上下文操作，放在画布上的浮动选择工具栏，卡片本身只展示状态。
...
- **M2 Library**：声音墙、声音相册、Like/评分、来源元数据筛选、自然语言搜索、人物/声音、
  ...
  - 首个浏览切片（2026-08-09）：声音墙成为默认首屏；资料库、横向声景卡片和详情检查器拆分
    ...
    所选声音的用户事实操作进入右下浮动工具栏，卡片密度开始驱动波形分辨率与渐进信息呈现。
