...
class PlaybackSession::Impl {
  public:
    Impl(const std::string& path, PlaybackAdjustment adjustment, PlaybackPipelineOptions options) :
        ...
        ring_ = std::make_unique<PlaybackFrameRing>(kRingCapacityFrames, channel_count_);
    ...
    void update_creative_vfx(CreativeVfxAdjustment adjustment) {
        ...
        EffectProcessingChain::validate_stereo_vfx(
            adjustment.stereo,
            kCanonicalSampleRate,
            channel_count_
        );
        EffectProcessingChain::validate_beat_repeat_vfx(
            adjustment.beat_repeat,
            kCanonicalSampleRate,
            channel_count_
        );
  ...
  private:
    ...
    void apply_pending_effect_updates() {
        ...
        if (creative_vfx_update_pending_) {
            ...
            effect_chain_->update_stereo_vfx(pending_creative_vfx_.stereo);
            effect_chain_->update_beat_repeat_vfx(pending_creative_vfx_.beat_repeat);
    ...
    std::unique_ptr<PlaybackFrameRing> ring_;
