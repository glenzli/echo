...
std::optional<echo::audio::ReverbAdjustment> ReverbProjection::fromQml(const QVariantMap& value) {
    ...
    if (mix < 0 || mix > 100 || pre_delay < 0 || pre_delay > 200 || decay < 100 || decay > 12000
        || size < 10 || size > 100 || damping < 0 || damping > 100 || low_cut < 20 || low_cut > 1000
        || high_cut < 1000 || high_cut > 20000 || low_cut >= high_cut) {
        return std::nullopt;
    }
